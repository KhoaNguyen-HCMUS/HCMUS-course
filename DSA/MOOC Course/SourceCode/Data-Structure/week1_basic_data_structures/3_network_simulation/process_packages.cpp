...
class Buffer {
 public:
  ...
  Response Process(const Request &request) {
    // Remove processed requests from the buffer
    while (!finish_time_.empty() &&
           finish_time_.front() <= request.arrival_time) {
      finish_time_.pop();
    }

    // Check if the buffer has space for the new request
    if (finish_time_.size() == size_) {
      return {-1, true};  // Buffer is full, drop the request
    }

    // Calculate the start time for the new request
    int start_time =
        finish_time_.empty() ? request.arrival_time : finish_time_.back();
    finish_time_.push(start_time + request.process_time);

    return {start_time, false};  // Successfully processed the request
  }
